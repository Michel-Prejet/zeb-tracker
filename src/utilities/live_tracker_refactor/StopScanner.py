...
from utilities.live_tracker_refactor.winnipeg_transit_gtfs.file_readers.StopTimesReader import parse_gtfs_time
...
class StopScanner:
    ...
    def scan_all_stops_and_record_observations(self) -> None:
        ...
        in this observation dictionary. The scan is done concurrently.
        """
    ...
    def _add_observations_from_stop_api_data(self, stop_info: dict) -> None:
        ...
        bus tracking number, route, destination, and scheduled/estimated arrival
        times.
        """
        ...
        for bus_obs in stop_info["buses"]:
            ...
            scheduled_arrival_time = parse_gtfs_time(bus_obs["arrivals"]["scheduled"].split("T")[1])
            estimated_arrival_time = parse_gtfs_time(bus_obs["arrivals"]["estimated"].split("T")[1])
