...
class ObservationDict:
    ...
    def get_closest_observation_for_bus(self, bus_tracking_num: int) -> BusObservation | None:
        """
        :param bus_tracking_num: the tracking number of the bus for which to
        retrieve the observation with the earliest estimated arrival time.
        :return: the observation with the earliest arrival time for the given
    ...
    def add_observation(self, observation: BusObservation) -> None:
        ...
        is sorted by increasing estimated arrival time.
        Assumes that no observation has already been added with the same
        tracking number, stop ID, and scheduled arrival time.
        ...
        bisect.insort(self.bus_observations[tracking_num], observation,
                      key=lambda obs: obs.estimated_arrival)
